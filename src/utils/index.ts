...
export function marketWatchToFavorite(
    favList = [],
    tradingMode = TRADING_MODE.EXCHANGE,
    marketWatch,
    isFutureDataOrigin = false
) {
    if (!favList || !favList.length || !marketWatch) return [];

    if (tradingMode === TRADING_MODE.EXCHANGE) {
        return (
            Array.isArray(marketWatch) &&
            marketWatch.length &&
            marketWatch.filter((m) => favList.includes(`${m?.bi}_${m?.qi}`))
        );
    }
    if (tradingMode === TRADING_MODE.FUTURES) {
        ...
        if (isFutureDataOrigin) {
            return (
                Array.isArray(marketWatch) &&
                marketWatch.length &&
                marketWatch.filter((m) => favList.includes(`${m?.b}_${m?.q}`))
            );
        } else {
...
export const sparkLineBuilder = (symbol, color) => {
    ...
    }/api/v1/chart/sparkline?symbol=${symbol}&broker=NAMI_SPOT&color=%23${color?.replace('#', '')}`;
};
...
export const getSupportCategories = async (language = 'vi') => {
    ...
    if (allTags) {
        const announcementCategories = allTags
            ...
            ?.filter(
                (f) => !CATEGORIES_IGRONED.includes(f.slug) && f.slug.startsWith(`noti-${language}`)
            );
        const faqCategories = allTags
            ...
            ?.filter(
                (f) => !CATEGORIES_IGRONED.includes(f.slug) && f.slug.startsWith(`faq-${language}`)
            );
...
export const getLastedArticles = async (
    tag = '',
    limit = 10,
    language = 'vi',
    isHighlighted = false
...
export const querySupportArticles = async (
    tag,
    keyword = '',
    language = 'vi',
    currentPage,
