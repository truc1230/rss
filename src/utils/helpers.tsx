...
export function getLoginUrl(mode = 'sso', action = 'login', options = {}) {
    ...
    if (typeof window !== 'undefined') {
        ...
        switch (mode) {
            case 'sso':
                if (action === 'register') {
                    return `${process.env.NEXT_PUBLIC_API_URL?.replace('/en', '').replace(
                        '/vi',
                        ''
                    )}/register/nami?${qs.stringify(params)}`;
                    // return `${___DEV___ ? 'https://auth-test.nami.trade' : 'https://auth.nami.io'}/register?${qs.stringify(params)}`;
                }
                return `${process.env.NEXT_PUBLIC_API_URL?.replace('/en', '').replace(
                    '/vi',
                    ''
                )}/login/nami?${qs.stringify(params)}`;
    ...
    if (walletType === WalletType.SPOT) {
        switch (action) {
            case EXCHANGE_ACTION.DEPOSIT:
                return `${PATHS.WALLET.EXCHANGE.DEPOSIT}?type=${payload?.type || 'crypto'}&asset=${
                    payload?.asset || 'USDT'
                }`;
            case EXCHANGE_ACTION.WITHDRAW:
                return `${PATHS.WALLET.EXCHANGE.WITHDRAW}?type=${payload?.type || 'crypto'}&asset=${
                    payload?.asset || 'USDT'
                }`;
            default:
                return '';
...
export const getSymbolObject = (symbol) => {
    if (
        !symbol ||
        ['USDTVNDC', 'VNDCUSDT', 'VNDC/USDT', 'USDT/VNDC', 'VNDC/', 'USDT/'].includes(symbol)
    ) {
...
export const getLiquidatePrice = (order = {}, activePrice = 0) => {
    ...
    liquidatePrice =
        (_size * activePrice + quoteQty * feeRatio - margin) / (size * (_sign - feeRatio));
...
export const getSuggestSl = (side, activePrice = 0, leverage = 10, profitRatio = 0.6) => {
    if (side == VndcFutureOrderType.Side.BUY) {
        return (
            ((-profitRatio * activePrice) / leverage +
                activePrice * (1 + DefaultFuturesFee.NamiFrameOnus)) /
            (1 - DefaultFuturesFee.NamiFrameOnus)
        );
    } else {
        return (
            ((-profitRatio * activePrice) / leverage -
